...
export default function GenericCard(props: Props) {
    ...
    return (
        <div className={`w-full bg-neutral-900 p-5 rounded-xl drop-shadow-lg ${props.maxHeight || ''}`}>
            ...
            {props.subTitle && <h3 className={`mt-2 font-med text-xl ${props.subTitleAnimated ? `text-transparent bg-clip-text bg-gradient-to-l from-${props.subTitleColour.replaceAll('text-', '')} to-${props.subTitleToColour.replaceAll('text-', '')} animate-text-fast` : props.subTitleColour ? props.subTitleColour : 'text-neutral-400' }`} >{props.subTitle}</h3>}
